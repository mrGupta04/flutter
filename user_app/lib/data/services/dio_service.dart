...
class DioService {
  ...
  Future<void> _onRequest(
    ...
    if (path.contains('admin/')) {
      ...
      final patientToken = await TokenStorage.instance.getPatientToken();
      if (patientToken != null && patientToken.isNotEmpty) {
        options.headers['Authorization'] = 'Bearer $patientToken';
      }
    } else if (_usesPatientAuth(path)) {
      final patientToken = await TokenStorage.instance.getPatientToken();
      if (patientToken != null && patientToken.isNotEmpty) {
        options.headers['Authorization'] = 'Bearer $patientToken';
      }
  ...
  bool _usesPatientAuth(String path) {
    return path.contains('online-consult') ||
        ...
        path.contains('ambulance/emergency') ||
        path.contains('ambulance/scheduled') ||
        path.contains('ambulance/my-bookings') ||
        path.contains('ambulance/payments') ||
        path.contains('ambulance/nearby');
  }
  ...
  /// POST multipart form (registration with files).
  ...
  /// PUT multipart form (profile update with optional files).
