...
/// Provider password recovery: email → OTP → new password.
class ProviderForgotPasswordScreen extends ConsumerStatefulWidget {
  const ProviderForgotPasswordScreen({super.key, required this.providerType});

  final ProviderType providerType;

  @override
  ConsumerState<ProviderForgotPasswordScreen> createState() =>
      _ProviderForgotPasswordScreenState();
}

class _ProviderForgotPasswordScreenState
    extends ConsumerState<ProviderForgotPasswordScreen> {
  ...
  final _repo = ProviderAuthRepository();
  ...
  Future<void> _sendOtp() async {
    ...
    setState(() {
      _step = 1;
      _maskedEmail =
          data?['maskedEmail'] as String? ?? _emailController.text.trim();
  ...
  Future<void> _resetPassword() async {
    ...
    context.go(widget.providerType.loginRoute);
  }
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Enter the email linked to your ${widget.providerType.label.toLowerCase()} account. We will send a one-time code if it exists.',
