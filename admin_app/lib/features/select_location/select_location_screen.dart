...
import 'provider_saved_addresses_store.dart';
...
class _SelectLocationScreenState extends ConsumerState<SelectLocationScreen> {
  ...
  String? _selectedId;
  List<SavedPlaceModel> _addresses = [];
  ...
  Future<void> _reloadAddresses() async {
    final items = await ProviderSavedAddressStore.instance.load();
  ...
  Future<void> _openAdd({SavedPlaceModel? existing}) async {
  ...
  String? _distanceFor(SavedPlaceModel address) {
    if (_hereLat == null ||
        _hereLng == null ||
  ...
  Future<void> _share(SavedPlaceModel address) async {
    await Share.share(address.displayLine, subject: address.label);
  }

  Future<void> _more(SavedPlaceModel address) async {
    ...
    if (action == 'delete') {
      await ProviderSavedAddressStore.instance.delete(address.id);
      await _reloadAddresses();
      return;
    }
    await ProviderSavedAddressStore.instance.upsert(
      SavedPlaceModel(
        id: address.id,
        label: address.label,
