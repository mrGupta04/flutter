...
| Email verification fails / no code | Add SMTP settings to `backend/.env`. On Render free tier, SMTP is blocked — upgrade to a paid instance. Or use mock mode (`EMAIL_PROVIDER=mock`) and read OTP from server logs |
